...
// +----------------------------------------------------------------------+
//
// $Id: Tree.php,v 1.2 2005/06/02 20:25:48 eric_juden Exp $
//

require_once FLICKR_PEAR_PATH . '/XML/Parser.php';
require_once FLICKR_PEAR_PATH . '/XML/Tree/Node.php';
...
class XML_Tree extends XML_Parser
{
     ...
     */
    public function &addRoot($name, $content = '', $attributes = [])
    {
     ...
     * @deprecated
     */
    public function &add_root($name, $content = '', $attributes = [])
    {
     ...
     * @param int $pos        position of child to be inserted in its parents children-list
    ...
    */
     ...
     * @deprecated
     */
    public function remove_child($path, $pos)
    {
    ...
    */
    ...
    {
        $clone = new self($this->filename, $this->version);
        ...
        foreach ($temp as $varname => $value) {
            if (!in_array($varname, ['filename', 'version', 'root'], true)) {
                $clone->$varname = $value;
            }
        }
